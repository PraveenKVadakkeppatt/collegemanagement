...
from college_app.models import  course,student,usermember,User
...
def tutor_welcome(request):
    return render(request,'tutor_welcome.html')
...
def show_student(request):
    ...
    return render(request,'show_student.html',{'dataread':context})
...
def profile(request):
    ...
    return render(request,'show_profile.html',{'u':user})

@login_required(login_url='loginpage')
def edit_page(request,pk):
    students=student.objects.get(id=pk)
    courses=course.objects.all()
    return render(request,'edit.html',{'students':students,'courses':courses})
...
def show_tutor(request):
    ...
    return render(request,'show_tuters.html',{'c':c})
...
def edit(request):
    if request.user.is_authenticated:
        ...
        return render(request,'edit_profile.html',{'users':user1})
    return redirect('/')   
...
def profile(request):
    if request.user.is_authenticated:
        ...
        print (current_user.id)
        user1=usermember.objects.get(user_id=current_user)
        return render(request,'show_profile.html',{'users':user1})